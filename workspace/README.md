...
4. 공통 영역에서 삭제되지 않는 두 서브워크플로 JSON을 내려받아 n8n에 가져옵니다.
5. n8n 연동 실습에서는 워크플로가 팀의 업로드 파일을 읽어 처리합니다.
6. n8n이 결과 파일을 올리면 `완료된 결과`에서 바로 열거나 내려받습니다.

`내 자료`와 `완료된 결과`의 파일 카드에는 업로드 시각이 초 단위 KST로 표시됩니다.

팀별 작업공간에는 해당 과제의 준비·처리·결과를 묶은 마인드맵이 표시됩니다. 참가자는 항목을 체크하거나 `내용 편집`에서 문구와 항목을 수정할 수 있으며, 저장한 상태는 팀별로 분리되어 Docker 데이터 볼륨에 유지됩니다.

작업공간은 공개 주소로 제공되므로 회사 내부 데이터와 개인정보를 업로드하지 않습니다. 실습에 필요한 정보는 반드시 마스킹하거나 제외합니다.
